...
export default function ProfilePage() {
  ...
  const handleResumeUpdated = (resumeUrl: string) => {
    if (!user || !user.freelancerProfile) return;
    setUser({
      ...user,
      freelancerProfile: {
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-[32px] border border-slate-200 bg-white p-8 shadow-xl">
        <div className="absolute inset-0 opacity-60" aria-hidden>
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.08),_transparent_60%)]" />
        </div>
        <div className="relative z-10 grid gap-8 lg:grid-cols-[1.2fr,auto]">
          <div className="space-y-4 text-slate-900">
            <p className="text-xs uppercase tracking-[0.6em] text-slate-500">Module 1 · Profile {profileComplete ? 'ready' : 'in progress'}</p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Badge variant="secondary" className="bg-slate-100 text-slate-700 uppercase tracking-[0.3em]">
                {role.toLowerCase()}
              </Badge>
